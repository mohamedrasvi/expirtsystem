...
$SQL_COMPLAINS_COUNT = "SELECT count(id) as tot FROM tbl_complains ";
$RS_COMPLAINS_COUNT = mysql_query($SQL_COMPLAINS_COUNT) or die(mysql_error());
$ROW_BRAND_COUNT = mysql_fetch_assoc($RS_COMPLAINS_COUNT);
...
$SQL_COMPLAINS = "SELECT * FROM tbl_complains " . $limit;
$RS_COMPLAINS = mysql_query($SQL_COMPLAINS) or die(mysql_error());
?>
...
<div id="page-content">
    ...
    <ul id="nav-info" class="clearfix">
        ...
        <li class="active"><a href="complains.php">All Complains</a></li>
    </ul>
    <!-- END Navigation info -->
    ...
    <table class="table">
        <thead>
            <tr>
                ...
                <th class="hidden-phone">Submitted From</th>
                <th class="hidden-phone">Submitted By</th>
            </tr>
        </thead>
        <tbody>
            <?php
            $row_id = 1;
            while ($COMPLAIN_ROW = mysql_fetch_array($RS_COMPLAINS)) {
                ?>
                <tr>
                    <td class="span1 text-center"><?php print($row_id); ?></td>
                    <td>
                        <?php 
                          $SQL_LINK = "SELECT * FROM tbl_search_results WHERE id = '".$COMPLAIN_ROW['link_id']."'";
                        $RS_LINK = mysql_query($SQL_LINK) or die(mysql_error());
                        $ROW_LINK = mysql_fetch_assoc($RS_LINK);
                        print($ROW_LINK['result_from_link']); 
                        ?>
                    </td>
                     <td><?php print($COMPLAIN_ROW['message']); ?></td>
                    <td class="hidden-phone"><?php print($COMPLAIN_ROW['date_and_time']); ?></td>
                    <td class="hidden-phone"><?php print($COMPLAIN_ROW['ip_address']); ?></td>
                    <td class="hidden-phone">
                        <?php 
                        $SQL_USER = "SELECT * FROM tbl_members WHERE id = '".$COMPLAIN_ROW['reported_by']."'";
